...
structure_provider = DynamicProvider(
    ...
    elements=['Block', 'Department', 'Division', 'Branch', 'Unit']
)
...
def file_db(count, pos, year, salary, parent_min, parent_max):
    """
    ...
    with Employee.objects.disable_mptt_updates():
        seeder.add_entity(Employee, count, {
            ...
            # 'parent_id': lambda x: Employee.objects.get(pk=random.randint(parent_min, parent_max)),
        })
        seeder.execute()
...
def run():
    ...
    """
    first_rec()
    file_db(50000, 'Head', 2022, 1000, 1, 1000)
    Employee.objects.rebuild()
    pass
